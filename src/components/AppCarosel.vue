...
export default{
  ...
  computed: {
    ...
    carouselWidth() {
      if(this.isMounted === true) return (this.$refs.widthImg[0].clientWidth) * this.itemsPerPage;
    }
  },
  methods: {
    updateImgPage(){
    ...
    nextSlide() {
      ...
      if (this.currentIndex < this.totalItems - this.itemsPerPage) {
        ...
        this.currentIndex = 0
        console.log(this.currentIndex)
        this.translateX += this.carouselWidth + (this.$refs.widthImg[0].clientWidth);
      }
    },
    prevSlide() {
  ...
  <div id="my_wrapper_carosel">
    ...
    <div class="my_carousel">
      <div class="my_carousel-inner" :style="{ transform: 'translateX(' + translateX + 'px)' }">
        <div v-for="(image, index) in eventImage" :key="index" class="carousel_my_item" ref="widthImg">
          <div class="my_div">
